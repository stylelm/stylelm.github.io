...
function generateEsEn(tableId) {
  ...
  let ext = ['_source.wav', '_cvsst.wav', '_s2ut.wav', '_ms.wav', '_stylelm.wav'];
...
function generateVC(tableId) {
  let table = document.getElementById(tableId);
  let ext = ['_source.wav', '_s2ut.wav', '_ppgvc.wav', '_nansy.wav', '_yourtts.wav', '_stylelm.wav', '_cvsst.wav'];

  for (var i = 0; i < 4; i++) {
    generateExampleRow(table.rows[1 + i], 'data/vc/es_'  + i, ext, 0);
  }

  for (var i = 0; i < 4; i++) {
    generateExampleRow(table.rows[5 + i], 'data/vc/fr_'  + i, ext, 0);
  }
}
